...
local function enum_index(emobj, field)
    ...
    if not value then
        error("the enum field is nil:{}.{},from:{}", emobj.__name, field, hive.where_call())
...
local function enum_newindex(emobj, field, value)
    ...
    if vlist[field] then
        error("enum {} redefine field {}!", emobj.__name, field)
    end
    vlist[field] = value
...
local function new(ems, name, base, ...)
    ...
    if eobj then
        if eobj.__source ~= source then
            error("enum {} redefined! source:{}", name, source)
        end
    else
